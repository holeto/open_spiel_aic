...
class MuZeroLeducInitConstants:
  ...
  final_chance_probs: chex.Array = () # Float[H, A]
  final_chance_continuations: chex.Array = () # Float[H, A]
  
  mvs_iset_map: chex.ArrayTree = () # ID -> Abstract iset
  
  mvs_history_action_utility: chex.Array = () # Float[H, A]
  mvs_history_iset: chex.Array = () # Int[H, Pl]
  mvs_history_actions: chex.Array = () # Int[H, Pl, A]
 
  
class MuZeroLeducInit:
  def __init__(self, constants: MuZeroLeducInitConstants):
    ...
    self.mvs_regrets = [jnp.zeros((self.constants.mvs_iset_map[pl].shape[0], self.constants.mvs_history_actions.shape[-1])) for pl in range(self.players)]
    self.mvs_averages = [jnp.zeros((self.constants.mvs_iset_map[pl].shape[0], self.constants.mvs_history_actions.shape[-1])) for pl in range(self.players)]
    self.mvs_cf_values = [jnp.zeros((self.constants.mvs_iset_map[pl].shape[0],)) for pl in range(self.players)]
  ...
  def check_constants(self, constants: MuZeroLeducInitConstants):
    ...
    assert len(constants.depth_iset_map) == self.max_depth, "Depth iset map has to be defined for each depth"
  ...
  def step(self):
    ...
    if self._alternating_updates:
      for player in range(self.players):
        self.regrets, self.averages, self.cf_values, self.mvs_regrets, self.mvs_averages, self.mvs_cf_values, self.last_depth_reaches = self.jit_step(
            self.regrets, self.averages, self.cf_values, self.mvs_regrets, self.mvs_averages, self.mvs_cf_values, averaging_coefficient, player, self.timestep
        )
  ...
  def find_reaches(self, strategies):
    ...
    final_reaches = history_reaches[-1][..., None] * self.constants.final_chance_probs
    return history_reaches, final_reaches.reshape((2, -1))
  ...
  def jit_step(self, regrets, averages, cf_values, mvs_regrets, mvs_averages, mvs_cf_values, average_policy_update_coefficient, player, iteration):
    ...
    current_mvs_strategies = [self.regret_matching(mvs_regrets[pl], jnp.ones_like(mvs_regrets[pl])) for pl in range(self.players)]

    history_mvs_p1_strategy = current_mvs_strategies[0][self.constants.mvs_history_iset[0]]
    history_mvs_p2_strategy = current_mvs_strategies[1][self.constants.mvs_history_iset[1]]
    history_mvs_strategy = jnp.stack([history_mvs_p1_strategy, history_mvs_p2_strategy], axis=0)
    
    mvs_action_value = self.constants.mvs_history_action_utility
    mvs_action_probabilities = history_mvs_strategy[0,..., None] * history_mvs_strategy[1, :, None, ...]
    mvs_value = jnp.sum(mvs_action_value * mvs_action_probabilities, axis=(-1, -2))
    
  

    after_chance_reaches = history_reaches[-1][..., None] * self.constants.final_chance_probs[None, ...]
    mvs_reaches = jnp.reshape(after_chance_reaches, (2, -1))
  
    mvs_action_realizations = mvs_reaches[..., None] * history_mvs_strategy
    if player != 1:
      iset_mvs_realizations = jnp.bincount(self.constants.mvs_history_actions[0].ravel(), mvs_action_realizations[0].ravel(), length=self.constants.mvs_history_actions.shape[-1] * self.constants.mvs_iset_map[0].shape[0]).reshape(mvs_averages[0].shape)
      
      # Sum across opponents current strategy.
      p1_mvs_value = jnp.sum(mvs_action_value * history_mvs_strategy[1, :, None, ...], axis=-1)
      mvs_cf_regret = (p1_mvs_value - mvs_value[..., None]) * jnp.expand_dims(mvs_reaches[1], -1)
      
      bin_mvs_regrets = jnp.bincount(self.constants.mvs_history_actions[0].ravel(), mvs_cf_regret.ravel(), length=self.constants.mvs_history_actions.shape[-1] * self.constants.mvs_iset_map[0].shape[0]).reshape(mvs_regrets[0].shape)
      
      cf_mvs_value = mvs_value * mvs_reaches[1]
      bin_mvs_value = jnp.bincount(self.constants.mvs_history_iset[0], cf_mvs_value, length=self.constants.mvs_iset_map[0].shape[0]).reshape(mvs_cf_values[0].shape)
      
      bin_mvs_reaches = jnp.bincount(self.constants.mvs_history_iset[0], mvs_reaches[1], length=self.constants.mvs_iset_map[0].shape[0]).reshape(mvs_cf_values[0].shape)
      
      bin_cf_value = jnp.where(bin_mvs_reaches > 1e-8, bin_mvs_value / bin_mvs_reaches, bin_mvs_value)
           
      mvs_cf_values[0] = mvs_cf_values[0] + (bin_cf_value - mvs_cf_values[0]) * (2 / (iteration + 1))
      mvs_regrets[0] = jnp.maximum(mvs_regrets[0] + bin_mvs_regrets, 0.0)
      mvs_averages[0] = mvs_averages[0] + iset_mvs_realizations * average_policy_update_coefficient
      
    if player != 0:
      iset_mvs_realizations = jnp.bincount(self.constants.mvs_history_actions[1].ravel(), mvs_action_realizations[1].ravel(), length=self.constants.mvs_history_actions.shape[-1] * self.constants.mvs_iset_map[1].shape[0]).reshape(mvs_averages[1].shape)
      
      p2_mvs_value = jnp.sum(mvs_action_value * history_mvs_strategy[0,..., None], axis=-2)
      mvs_cf_regret = (p2_mvs_value - mvs_value[..., None]) * jnp.expand_dims(mvs_reaches[0], -1)
      
      bin_mvs_regrets = jnp.bincount(self.constants.mvs_history_actions[1].ravel(), mvs_cf_regret.ravel(), length=self.constants.mvs_history_actions.shape[-1] * self.constants.mvs_iset_map[1].shape[0]).reshape(mvs_regrets[1].shape)
      
      cf_mvs_value = mvs_value[..., None] * jnp.expand_dims(mvs_reaches[0], -1)
      bin_mvs_value = jnp.bincount(self.constants.mvs_history_iset[1].ravel(), cf_mvs_value.ravel(), length=self.constants.mvs_iset_map[1].shape[0]).reshape(mvs_cf_values[1].shape)
      
      bin_mvs_reaches = jnp.bincount(self.constants.mvs_history_iset[1].ravel(), mvs_reaches[0].ravel(), length=self.constants.mvs_iset_map[1].shape[0]).reshape(mvs_cf_values[1].shape)
      
      bin_cf_value = jnp.where(bin_mvs_reaches > 1e-8, bin_mvs_value / bin_mvs_reaches, bin_mvs_value)        
      
      mvs_cf_values[1] = mvs_cf_values[1] + (bin_cf_value - mvs_cf_values[1]) * (2 / (iteration + 1))
      mvs_regrets[1] = jnp.maximum(mvs_regrets[1] - bin_mvs_regrets, 0.0)
      mvs_averages[1] = mvs_averages[1] + iset_mvs_realizations * average_policy_update_coefficient
      

    chance_action_value = mvs_value.reshape((-1, 4)) # 4 is a constant for chance nodes
    before_chance_utils = jnp.sum(chance_action_value * self.constants.final_chance_probs, axis=-1)
    
    # How to work with depth_utils in the first round (and subsequent)
    depth_utils = [before_chance_utils]
    ...
    for d in range(self.max_depth - 1, -1, -1):
      ...
      # history_value = jnp.sum(action_value *)
    return regrets, averages, cf_values, mvs_regrets, mvs_averages, mvs_cf_values, history_reaches[-1]
