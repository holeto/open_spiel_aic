...
class MuZeroLeducCFRConstants:
  ...
  depth_history_is_chance: chex.ArrayTree = () # Bool[D, H(D)]
 
  
  
class MuZeroLeducCFR:
  def __init__(self, constants: MuZeroLeducCFRConstants, depth_iset_map: chex.ArrayTree):
  ...
  def check_constants(self, constants: MuZeroLeducCFRConstants):
    ...
    assert len(constants.depth_history_is_chance) == self.max_depth, "Depth history is chance has to be defined for each depth"
    ...
    for d in range(self.max_depth):
      ...
      assert constants.depth_history_is_chance[d].shape[0] == histories_in_depth
  ...
  def jit_step(self, regrets, averages, cf_values, average_policy_update_coefficient, player, iteration):
    ...
    for d in range(self.max_depth - 1, -1, -1):
      ...
      if player != 1:
        ...
        p1_cf_regret = jnp.where(self.constants.depth_history_is_chance[d][..., None], 0, p1_cf_regret)
      ...
      if player != 0:
        ...
        p2_cf_regret = jnp.where(self.constants.depth_history_is_chance[d][..., None], 0, p2_cf_regret)
