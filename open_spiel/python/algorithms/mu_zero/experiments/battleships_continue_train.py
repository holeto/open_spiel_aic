...
# Training setting
parser.add_argument("--model_path", type=str, default="muzero_networks/battleships_2x2_2/seed_43/muzero_100.pkl", help="Model path") 
...
def main(): 
  ...
  assert isinstance(model.game, JaxBattleships)
  
  ship_sizes_str = "_".join([str(size) for size in model.game.ship_sizes])
  
  folder = args.save_folder + "/battleships_" + str(model.game.board_shape[0]) + "x" + str(model.game.board_shape[1]) + "_" + ship_sizes_str + "/" + "seed_" + str(model.config.seed) + "/" + "no_rnad_train/" 
