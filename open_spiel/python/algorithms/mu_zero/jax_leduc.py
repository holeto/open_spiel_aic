...
class JaxOriginalLeduc:
  ...
  def _jit_initialize_structures(self, key):
    ...
    return action_history, public_card, jax.random.choice(key, private_cards, axis=0), current_chips,key, init_reaches, jnp.stack([p1_legal_mask, p2_legal_mask], axis=0)
  ...
  def _jit_batch_initialize_structures(self, batch, key):
    ...
    #private_cards = jnp.repeat(jnp.concatenate([p1_private_cards, p2_private_cards], axis=1), [self.public_chance_outcomes, 1])
    public_card = jnp.zeros([batch, 1])
    #[Batch, Pl, A]
    batch_legals = jnp.tile(jnp.stack([p1_legal_mask, p2_legal_mask], axis=0), (batch, 1, 1))
    return action_history, public_card, chosen_cards, current_chips, key, init_reaches, batch_legals

  def initialize_structures(self, key, batch=1):
    if batch ==1:
      action_history, public_card, chosen_private_cards, current_chips, key, init_reaches, legals = self._jit_initialize_structures(key)
    else:
      action_history, public_card, chosen_private_cards, current_chips, key, init_reaches, legals = self._jit_batch_initialize_structures(batch, key)   
    ...
    return action_history, public_card,  chosen_private_cards, current_chips, key, init_reaches, legals
  ...
  @functools.partial(jax.jit, static_argnums=(0))
  def apply_action(self, action_history, public_card, private_cards, current_chips, key, actions, round, turns_this_round, turn):
    ...
    next_player = (self.starting_player + ((turns_this_round + 1) % 2)) % self.players
    ...
    num_raises = jnp.sum(action_history[..., RAISE_ID - 1])
    ...
    new_acting_legals = jnp.where(num_raises < self.max_raises_per_round * (round + 1), new_acting_legals, new_acting_legals - raise_oh)
    ...
    reward = jnp.where(terminal, jnp.where(jnp.logical_and(tie, ~folded), 0, ((1 - 2 * winner) * current_chips[1-winner])), 0)
    reward = jnp.where(reward == 0, reward, jnp.where(round > 0, reward / self.chance_outcomes, reward / self.private_chance_outcomes))


    return action_history, public_card, private_cards, current_chips, terminal, jnp.asarray([reward, -reward]), round, turns_this_round + 1, key, new_legals